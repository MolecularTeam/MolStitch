...
class DockingVina(object):
    def __init__(self, target):
        ...
        self.vina_program = os.path.join(path_here, 'qvina02')
        self.receptor_file = os.path.join(path_here, f'{target}.pdbqt')
...
class QuickVina2:
    ...
    def __init__(self, target):
        self.vina_oracle = DockingVina(target)
        self.name = target

    def __call__(self, smiles: list) -> np.ndarray[float]:
